...
NuevaWriteArgs(char **args)
{
    ...
    for (i = 1; i < MAX_ARG_COUNT; i++) {
        ...
        if(i<=4) machine->WriteRegister(3+i, sp);
        // la direccion los primeros 4 argumentos (sin incluir el 
        // primero que es el nombre del programa) los enviamos a los 
        // registros a0-a3 
    }
    ASSERT(i < MAX_ARG_COUNT);
    
    sp -= sp % 4;     // Align the stack to a multiple of four.
    sp -= 4;  // Make room for the trailing null.
    if(i>4){
    sp -= (i-4) * 4;  // Make room for the array.
    ...
    machine->WriteMem(sp + 4 * (i-4), 4, 0);  // The last is null.
