...
public:
    ...
    /// Initialize a `Thread`.
    Thread(const char *debugName, bool usojoin = false, unsigned prio = prioinicial);
    ...
    void SetPrio(unsigned prio);

    void Print() const;
    
    ///Retorna el estado con el que terminó el hijo.
    int Join();
...
#ifdef USER_PROGRAM
	/// Es la tabla de archivos abiertos
	Table<OpenFile*> *tablarch;
...
public:
	...
	OpenFileId AddArchivo(OpenFile *archivo);
	
	void DeleteArchivo(OpenFileId id);
