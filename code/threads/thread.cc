...
Thread::Thread(const char *threadName, bool usojoin, unsigned prio)
{
    name      = threadName;
    stackTop  = nullptr;
    stack     = nullptr;
...
#ifdef USER_PROGRAM
  // Cerramos los archivos si el proceso no lo hizo
  while(!tablarch->IsEmpty()){
		int i=0;
		for(;;i++){
			if(tablarch->HasKey(i)){
				delete tablarch->Remove(i);
				break;
			}
		}
	}
...
void 
Thread::SetPrio(unsigned prio)
{
    ASSERT (prio >= 0 && prio < colas);
    this->prioridad = prio;
}
...
OpenFileId
Thread::AddArchivo(OpenFile *archivo)
{
	return tablarch->Add(archivo);
}

void
Thread::DeleteArchivo(OpenFileId id)
{
	if(tablarch->HasKey(id))
		tablarch->Remove(id);
}

OpenFile*
Thread::GetArchivo(OpenFileId id)
{
	if(tablarch->HasKey(id))
		return tablarch->Get(id);
	return nullptr;
}
#endif
